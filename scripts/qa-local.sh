...
ok("admin_oauth", "auth_methods: [oauth]" in admin or "oauth" in admin)
ok("admin_no_git_gateway", "git-gateway" not in admin)
ok("admin_posts_archive_dir", Path("_posts/archive").is_dir())
ok("admin_cms_posts_root_count", len(list(Path("_posts").glob("*.md"))) == 10)
ok("admin_archived_posts_count", len(list(Path("_posts/archive").glob("*.md"))) == 19)
admin_index = read("_site/admin/index.html")
ok("admin_sveltia", "@sveltia/cms@0/dist/sveltia-cms.js" in admin_index)
ok("admin_no_caret_cdn", "@^" not in admin_index)
ok("admin_no_decap", "decap-cms" not in admin_index)
...
ok("archived_belaku_permalink", Path("_site/news/2019/11/03/belaku/index.html").exists())
