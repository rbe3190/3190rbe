...
ok("no_publish_dir_redirects_file", not Path("_redirects").exists() and not Path("_site/_redirects").exists())
...
ok("no_consent_banner", "rbe-consent" not in about and "consent-banner" not in css)
ok("no_clarity", "clarity.ms" not in site_js)
ok("ga_default_on", "gtag/js" in site_js)
ok("no_analytics_opt_out", "data-analytics-opt-out" not in read("_site/privacy.html") and "rbe-analytics-consent" not in site_js)
...
ok("admin_default_sort_events_start", 'field: start, default_sort: desc' in admin)
ok("admin_default_sort_news_date", 'field: date, default_sort: desc' in admin)
...
ok("admin_github_backend", "name: github" in admin)
ok("admin_github_repo", "rotaractblreast/RBEwebsite" in admin)
ok("admin_no_auth_methods", "auth_methods:" not in admin)
ok("admin_no_git_gateway", "git-gateway" not in admin)
ok("admin_no_posts_archive_dir", not Path("_posts/archive").exists())
ok("admin_cms_posts_root_count", len(list(Path("_posts").glob("*.md"))) == 29)
admin_index = read("_site/admin/index.html")
ok("admin_decap", "decap-cms@" in admin_index)
ok("admin_no_sveltia", "@sveltia/cms" not in admin_index)
ok("admin_no_identity_widget", "netlify-identity-widget" not in admin_index)
...
# Single H1 on a sample post if present (permalink: news/:year/:month/:day/:title/)
posts = sorted(Path("_site").glob("news/[0-9][0-9][0-9][0-9]/[0-9][0-9]/[0-9][0-9]/*/index.html"))
